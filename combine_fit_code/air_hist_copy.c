    ...
    TString simROOTfilename="smeared_rec_CryMuAna_0_749.root")
{
    ...
    /*h_mu->Scale(((1.0)/mu_count)*0.351485);
    h_e->Scale(((1.0)/e_count)*0.525415);
    h_other->Scale(((1.0)/other_count)*0.1231);*/

    /*h_mu->Scale(((1.0)/mu_count)*0.351485*exp_count);
    h_e->Scale(((1.0)/e_count)*0.525415*exp_count);
    h_other->Scale(((1.0)/other_count)*0.1231*exp_count);*/

    h_mu->Scale(((1.0)/h_mu->Integral())* 0.345498*h_obs->Integral()/exp_count);
    h_e->Scale(((1.0)/h_e->Integral())*0.58425*h_obs->Integral()/exp_count);
    h_other->Scale(((1.0)/h_other->Integral())*0.0702514*h_obs->Integral()/exp_count);
    ...
    legend->AddEntry(h_mu, Form("MC_mu (%.2f%% #pm 0.65%%)", ((h_mu->Integral() * 1.0)/(h_air->Integral() * 1.0)*100.0)), "l");
    legend->AddEntry(h_e, Form("MC_e (%.2f%% #pm 2.08%%)", (h_e->Integral() * 1.0)/(h_air->Integral() * 1.0)*100.0), "l");
    legend->AddEntry(h_other, Form("MC_other (%.2f%% #pm 1.69%%)", (h_other->Integral() * 1.0)/(h_air->Integral() * 1.0)*100.0), "l");
    legend->Draw();
