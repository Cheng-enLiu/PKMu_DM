    ...
    TString simROOTfilename="smeared_rec_CryMuAna_0_749.root")
{
    ...
    legend->AddEntry(h_mu, Form("MC_mu (%.2f%% #pm 0.35%%)", (h_mu->Integral() * 1.0)/(h_air->Integral() * 1.0)*100.0 ), "l");
    legend->AddEntry(h_e, Form("MC_e (%.2f%% #pm 0.35%%)", (h_e->Integral() * 1.0)/(h_air->Integral() * 1.0)*100.0), "l");
    legend->AddEntry(h_other, Form("MC_other (%.2f%% #pm 0.14%%)", (h_other->Integral() * 1.0)/(h_air->Integral() * 1.0)*100.0), "l");
    legend->Draw();
