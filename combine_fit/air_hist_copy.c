...
Int_t air_hist_copy(TString expROOTfilename="250212_63d_air.root",
    TString simROOTfilename="smeared_rec_CryMuAna_0_749.root")
{
    ...
    /*TH1F *h_air = new TH1F("h_air", "distribution of scattering angle #theta", 50, 0.1, 0.35);
    TH1F *h_obs = new TH1F("h_obs", "distribution of scattering angle #theta(air)", 50, 0.1, 0.35);
    TH1F *h_mu = new TH1F("h_mu", "distribution of scattering angle #theta", 50, 0.1, 0.35);
    TH1F *h_e = new TH1F("h_e", "distribution of scattering angle #theta", 50, 0.1, 0.35);
    TH1F *h_other = new TH1F("h_other", "distribution of scattering angle #theta", 50, 0.1, 0.35);*/
    ...
    for(i=0;i<nentries_sim;i++){
        ...
        //if(zs>100||zs<-100) continue;
        //if(xs>100||xs<-100) continue;
        //if(ys>100||ys<-100) continue;
        //if(angs>0.35||angs<0.1) continue; 
        if(abs(Pid)==11){
            h_e->Fill(angs);
        }
        if(abs(Pid)==13){
    ...
    /*h_mu->Scale(((1.0)/mu_count)*0.352605);
    h_e->Scale(((1.0)/e_count)*0.514695);
    h_other->Scale(((1.0)/other_count)*0.1327);*/

    /*h_mu->Scale(((1.0)/mu_count)*0.352605*exp_count);
    h_e->Scale(((1.0)/e_count)*0.514695*exp_count);
    h_other->Scale(((1.0)/other_count)*0.1327*exp_count);*/

    h_mu->Scale(((1.0)/h_mu->Integral())* 0.333518*h_obs->Integral()/exp_count);
    h_e->Scale(((1.0)/h_e->Integral())*0.656675*h_obs->Integral()/exp_count);
    h_other->Scale(((1.0)/h_other->Integral())*0.00980654*h_obs->Integral()/exp_count);

    h_obs->Scale(((1.0)/exp_count));
    ...
    legend->AddEntry(h_e, Form("MC_e (%.2f%% #pm 1.26%%)", (h_e->Integral() * 1.0)/(h_air->Integral() * 1.0)*100.0), "l");
    legend->AddEntry(h_other, Form("MC_other (%.2f%% #pm 0.98%%)", (h_other->Integral() * 1.0)/(h_air->Integral() * 1.0)*100.0), "l");
    legend->Draw();
